...
public class ProdottoDAO {
    ...
    public Prodotto getProdottoById(int idProdotto) throws SQLException {
        try (Connection connection = ConPool.getConnection();
             PreparedStatement stmt = connection.prepareStatement("SELECT * FROM prodotto WHERE id_prodotto = ? AND disabilitato = 0")) {
    ...
    public ArrayList <Prodotto> getProdottiByCategoria(String categoria) throws SQLException {
        try (Connection connection = ConPool.getConnection();
             PreparedStatement stmt = connection.prepareStatement("SELECT * FROM prodotto WHERE categoria = ? AND disabilitato = 0")) {
            ArrayList<Prodotto> prodotti = new ArrayList<>();
    ...
    public int addProdotto(Prodotto p) throws SQLException {
        try (Connection connection = ConPool.getConnection();
             PreparedStatement stmt = connection.prepareStatement("INSERT INTO prodotto VALUES (default, ?, ?, ?, ?, ?, ?, ?, ?, ?, default)")) {
    ...
    public ArrayList<Prodotto> getUltimiProdotti() throws SQLException {
        try (Connection connection = ConPool.getConnection();
             PreparedStatement stmt = connection.prepareStatement("SELECT * FROM prodotto WHERE disabilitato = 0 ORDER BY id_prodotto DESC LIMIT 8")) {
            ArrayList<Prodotto> prodotti = new ArrayList<>();
    ...
    public ArrayList<Prodotto> cercaProdotti(String nome) throws SQLException {
        try (Connection connection = ConPool.getConnection();
             PreparedStatement stmt = connection.prepareStatement("SELECT * FROM prodotto WHERE UPPER(CONCAT(marca, modello, descrizione, categoria)) LIKE UPPER(?) AND disabilitato = 0")) {
    ...
    public void eliminaProdotto(int idProdotto) throws SQLException {
        try (Connection connection = ConPool.getConnection();
             PreparedStatement stmt = connection.prepareStatement("UPDATE prodotto SET disabilitato = 1 WHERE id_prodotto = ?")) {
            stmt.setInt(1, idProdotto);
            stmt.executeUpdate();
        }
    }
    ...
    public int getLastProduct() throws SQLException {
        try (Connection connection = ConPool.getConnection();
             PreparedStatement stmt = connection.prepareStatement("SELECT MAX(id_prodotto) FROM prodotto WHERE disabilitato = 0;")) {
            ResultSet rs = stmt.executeQuery();
            rs.next();
    ...
    public void modificaProdotto(Prodotto p) throws SQLException{
        try (Connection connection = ConPool.getConnection()) {
            if(p.getImmagine() == null) {
                System.out.println("primo: " + p.getMarca() + " - " + p.getModello() + " " + p.getPrezzo() + " " + p.getDescrizione() + " " + p.getDimensioni() + " " + p.getPeso() + " " + p.getCategoria().getNomeCategoria() + " " + p.getId());
                try (PreparedStatement stmt = connection.prepareStatement("UPDATE prodotto SET marca = ?, modello = ?, prezzo = ?, descrizione = ?, dimensioni = ?, peso = ?, categoria = ? WHERE id_prodotto = ?")) {
                ...
                System.out.println("Secondo: " + p.getMarca() + " - " + p.getModello() + " " + p.getPrezzo() + " " + p.getDescrizione() + " " + p.getDimensioni() + " " + p.getPeso() + " " + p.getCategoria().getNomeCategoria() + " " + p.getId());
                try (PreparedStatement stmt = connection.prepareStatement("UPDATE prodotto SET marca = ?, modello = ?, prezzo = ?, descrizione = ?, dimensioni = ?, peso = ?, categoria = ?, immagine =? WHERE id_prodotto = ?")) {
                    stmt.setString(1, p.getMarca());
                    stmt.setString(2, p.getModello());
                    stmt.setDouble(3, p.getPrezzo());
                    stmt.setString(4, p.getDescrizione());
