...
Performance is a core non-functional requirement: beyond functional correctness, the system should remain responsive, predictable, and stable under typical workload conditions. In a dependability-oriented project, performance is not just a user-experience concern: if critical operations degrade or time out, performance becomes an availability issue. In addition, a backend that spends too much time per request is easier to overload, which increases exposure to denial-of-service scenarios.

The project checklist explicitly requires microbenchmarks with JMH (Java Microbenchmark Harness). For this reason, the work described in this chapter focuses on \emph{microbenchmarking} selected backend components rather than executing full end-to-end load tests. The intent is to isolate representative operations that are likely to dominate latency (data-access and service orchestration), measure them in a controlled way, and establish a baseline that can be compared across future changes.

\section{Methodology}
\subsection{Selection of Performance-Critical Scenarios}
The starting point is the identification of ``slow or expensive'' use cases at the backend level. Without production telemetry, the selection was engineering-driven: database interactions (queries and inserts) and service methods that combine validation with DAO calls often dominate request time. Therefore, the benchmark suite targets:
\begin{itemize}
...
\subsection{Running JMH (Jar-First Workflow)}
The lecture reference example follows a ``jar-first'' workflow (generate jar, list benchmarks, run selected benchmarks, export JSON). The same style is applied here because it is explicit, reproducible, and easy to document.
...
\section{Benchmark Design (Theory-to-Code Rationale)}
...
\hline
\texttt{AdminServiceBenchmark.benchmarkGetProdottoFound} & Happy path: retrieve existing product (ID 1), consumes result. \\
\texttt{AdminServiceBenchmark.benchmarkGetProdottoNotFound} & Error path: missing product (ID 99999), exceptions consumed. \\
\texttt{AdminServiceBenchmark.benchmarkInfoOrdineFound} & Happy path: retrieve existing order (ID 1), consumes result. \\
\texttt{AdminServiceBenchmark.benchmarkInfoOrdineNotFound} & Error path: missing order (ID 99999), exceptions consumed. \\
\texttt{OrdineDAOBenchmark.benchmarkGetOrdineByIdValid} & Happy path: DB fetch for order (ID 1). \\
\texttt{OrdineDAOBenchmark.benchmarkGetOrdineByIdInvalid} & Error path: invalid ID (\texttt{-1}), validation/exception path. \\
\texttt{OrdineDAOBenchmark.benchmarkGetProdottoOrdineNull} & Error path: null input, validation/exception path. \\
\texttt{SpecificheDAOBenchmark.benchmarkGetSpecificheValidId} & DB-dependent lookup for product ID 1 (catches \texttt{SQLException} if DB is down). \\
\texttt{SpecificheDAOBenchmark.benchmarkGetSpecificheInvalidId} & Invalid product ID (\texttt{-1}), exception path. \\
\texttt{UtenteDAOBenchmark.benchmarkGetUtenteByIdNotFound} & Attempt to find user ID \texttt{99999}, consumes returned value or \texttt{SQLException}. \\
\texttt{WishListDAOBenchmark.benchmarkGetWishListWithNullUtente} & Null user input, exception path. \\
\texttt{WishListDAOBenchmark.benchmarkGetWishListWithInvalidUtente} & Invalid user ID (\texttt{-1}), exception path. \\
\texttt{RecensioneDAOBenchmark.benchmarkAddRecensione} & DB insert attempt (side effect), exceptions consumed if thrown. \\
\texttt{LogoutServiceBenchmark.benchmarkLogoutValidSession} & Logout using a minimal \texttt{HttpSession} proxy; consumes session. \\
\texttt{ConnectionPoolBenchmark.benchmarkGetReleaseConnection} & Acquire/release a DB connection; requires DB availability (throws \texttt{SQLException}). \\
\hline
\end{tabular}
\end{table}
...
Benchmark names in the provided output match the updated repository version, therefore results are directly traceable to the benchmark code. The received snippet does not include \texttt{Cnt} and \texttt{Error}; a JSON export (\texttt{reports/performance/jmh\_result.json}) should be attached to complete the statistical fields.

\begin{table}[h]
\centering
\caption{Measured JMH results (AverageTime, ms/op) for the selected scope.}
\label{tab:jmh-results}
\begin{tabular}{p{0.44\textwidth} p{0.28\textwidth} p{0.10\textwidth}}
\hline
\textbf{Benchmark} & \textbf{Scenario} & \textbf{Score} \\
\hline
\texttt{AdminServiceBenchmark.benchmarkGetProdottoFound}    & Happy path (ID 1)     & 0.079 \\
\texttt{AdminServiceBenchmark.benchmarkGetProdottoNotFound} & Error path (missing)  & 0.139 \\
\texttt{AdminServiceBenchmark.benchmarkInfoOrdineFound}     & Happy path (ID 1)     & 0.216 \\
\texttt{AdminServiceBenchmark.benchmarkInfoOrdineNotFound}  & Error path (missing)  & 0.085 \\
\texttt{OrdineDAOBenchmark.benchmarkGetOrdineByIdValid}     & Happy path (ID 1)     & 0.104 \\
\texttt{OrdineDAOBenchmark.benchmarkGetOrdineByIdInvalid}   & Error path (ID -1)    & 0.001 \\
\texttt{OrdineDAOBenchmark.benchmarkGetProdottoOrdineNull}  & Error path (null)     & 0.001 \\
\hline
\end{tabular}
\end{table}
...
Recommended next steps (low-impact):
\begin{itemize}
  \item Ensure the JSON output file (\texttt{reports/performance/jmh\_result.json}) is archived as evidence and referenced in the report, including \texttt{Cnt} and \texttt{Error}.
  \item For final verification runs, increase \texttt{@Fork} (e.g., 2--3) to improve robustness, and repeat runs to check stability.
  \item Consider adding a \texttt{SampleTime} run configuration if tail latency (p95/p99) becomes a reporting requirement.
  \item For strict microbenchmark hygiene, consume returned values also on successful DB paths (even if the DB call itself prevents meaningful dead-code elimination in practice).
  \item For side-effect benchmarks (inserts), add optional cleanup/rollback or reset DB state between runs to reduce drift.
\end{itemize}

\section{Missing Data}
At this point, the run command and environment are documented. The remaining item is to ensure the JSON file (\texttt{reports/performance/jmh\_result.json}) is included as raw evidence (versioned in the repository or attached in the submission materials), so statistical fields (\texttt{Cnt}, \texttt{Error}) can be reported and the experiment can be independently verified.
